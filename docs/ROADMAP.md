...
| 6.9 | **`-march` image variants** (moved from 5.11): amd64 base image is built `-march=native` (portability hazard: SIGILL on CPUs older than the build host) while the solver's own objects have no `-march` at all (vector units unused in the hottest code). Publish psABI-level tags (x86-64-v2/v3, v4 if it measures; arm64 already portable `armv8-a`), same level in **both** layers (base libs + solver makefile `OPT`); teems-R picks the tag from host capability (`ld.so --help` lists supported levels), default v2. A/B gate per level on the bench rigs; NB bit-identity does not survive `-march` changes, so each variant needs its own golden anchoring + a cross-variant accuracy-level sanity check | coordinate with 6.6 (one retagging for rename + variants) |
