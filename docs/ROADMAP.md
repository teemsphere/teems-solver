...
| 6.3 | **Stepping methods** (in cost-benefit order): (a) Euler multi-step — near-free (Mmid driver with midpoint flag = 0), immediate parity gap; (b) Gragg terminal smoothing ✅ **verified already present** (2026-07-13, code inspection, no change needed): `Mmid` is the full Gragg method, not plain modified midpoint — per solution pass: Euler start (solve_drivers.c:1110), midpoint leapfrog z_{m+1}=z_{m−1}+2h·f(z_m) (`clag1+2*x1`, :1150; coefficient side via `updates_apply(midpoint=1)`), then a post-loop Jacobian rebuild+solve at the final state (:1289) feeding the smoothing ½(z_n+z_{n−1}+h·f(z_n)) (:1604 change / :1616 percent) — the (n+1)-th evaluation that manual §30.2 says distinguishes gragg from midpoint; corroborated by the 1/(r²−1) Richardson weights (:1699), valid only for the even-power (h²) error expansion the smoothing guarantees. No unsmoothed-midpoint option exists (`Mmid` is the sole multi-step method string; the name undersells the numerics) — if plain-midpoint parity is ever wanted it's a subtraction, not an addition. NB for (a): Euler's error series is in h, not h², so the h² extrapolation weights do NOT carry over to Euler multi-step; (c) Runge-Kutta: RK2/RK4/BoSha32/DoPri54 via Butcher-tableau driver — stages reuse the existing refill+solve primitive, compatible with all matrix methods; adaptive accept/reject must be MPI-collective (Allreduce the embedded-error norm) and the controller tuned conservatively at scale (a rejected step wastes a full set of block factorizations); (d) Adams-Bashforth — ONE factorization per step at order k (vs RK4's 4): directly attacks the factorization wall; RK startup; stability caveats; (e) implicit stiff methods (Radau/BDF/ode15s) — parked: performance-negative for smooth CGE paths; revisit only if adaptive RK step-size collapse demonstrates stiffness | RK gate: fixed-accuracy wall-time vs Gragg 2-4-8 |
